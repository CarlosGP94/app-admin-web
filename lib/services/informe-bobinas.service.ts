...
export interface BobinaReportRow {
  calidad_id?: number;
  calidad?: string;
  concepto?: string;
  unidades?: number;
  peso?: number; // Peso total en Toneladas (unidades * peso_medio)
}

export interface InformeBobinasOptions {
  ids?: number[];
  path?: string;
}

interface InformeFilaSegura extends BobinaReportRow {
...
function formatPeso(pesoInTn: number | null | undefined): string {
  if (pesoInTn == null || isNaN(pesoInTn)) return "0.00 Tn";
  return `${pesoInTn.toFixed(2)} Tn`;
}

/**
 * Obtiene los datos de bobinas desde la base de datos
 */
async function obtenerDatosInforme(
  pool: ConnectionPool,
  ids?: number[],
): Promise<BobinaReportRow[]> {
  const request = pool.request();
  const whereClauses: string[] = ["b.activa = 1", "b.unidades > 0"];
  ...
  if (ids && ids.length > 0) {
    ...
    whereClauses.push(`b.id IN (${paramNames.join(", ")})`);
  }
  ...
  const query = `
    SELECT 
      ...
      ISNULL(b.concepto, '-') AS concepto,
      ISNULL(b.unidades, 0) AS unidades,
      (ISNULL(b.unidades, 0) * ISNULL(b.peso_medio, 0)) AS peso
    FROM Bobinas b
  ...
  return (result.recordset as BobinaReportRow[]) || [];
}

/**
 * Genera el documento HTML completo listo para convertir a PDF
 */
function generarHtmlInforme(reportRows: BobinaReportRow[]): string {
  ...
  if (reportRows.length) {
    const groups = new Map<
      string,
      { calidad: string; rows: BobinaReportRow[] }
    >();
...
/**
 * Servicio para generar el informe de bobinas en PDF
 */
export async function informeBobinas({
  path: destinationPath,
  ids,
}: InformeBobinasOptions = {}): Promise<Buffer> {
  // Ajusta el nombre del pool de conexión si utilizas otra clave en tu helper (ej. "bobinas" o "tubos")
  const pool = await getConnection("tubos");
  const reportRows = await obtenerDatosInforme(pool, ids);

  if (reportRows.length === 0) {
    throw new Error(
      "No se encontraron bobinas activas con unidades disponibles en el inventario.",
    );
  }
