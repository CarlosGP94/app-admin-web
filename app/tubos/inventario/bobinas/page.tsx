...
export function BobinasView() {
  ...
  ): Promise<{ data: Bobina[]; total: number }> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            errorData.message ||
            "Error al generar el informe de bobinas.",
        );
      }
  ...
  return (
    ...
    >
      <ConfirmDialog
        ...
        message="¿Estás seguro de que deseas eliminar esta bobina? Esta acción no se puede deshacer."
...
const columns = (
  ...
  },
  {
    ...
    format: (row) => {
      return (
        <Chip
          color={row.activa ? "success" : "error"}
